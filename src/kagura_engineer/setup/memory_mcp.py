...
# The secret-bearing files setup may write into the repo. Mirrors the kagura
# SDK's own `_check_gitignore` secret_files list: `.mcp.json` (static-token form
# bakes a bearer key) and `.kagura.json` (holds the api_key, written under
# `--full`). Both are git-ignored before any write — see ensure_memory_mcp_config.
_SECRET_FILES = (".mcp.json", ".kagura.json")
...
def ensure_memory_mcp_config(
    ...
    *,
    ...
    # Fail-secure (gate2 cso + code-review, issue #35): git-ignore the secret
    # files BEFORE writing them. The static-token .mcp.json bakes in a bearer
    # key, and `--full` additionally writes .kagura.json holding the api_key. The
    ...
    for _secret in _SECRET_FILES:
        try:
            ...
            return _result(
                StepStatus.FAIL,
                f"could not gitignore {_secret}: {type(exc).__name__}: {exc}",
                fix_hint="ensure <repo>/.gitignore is writable; refusing to write a "
                "secret-bearing config un-ignored",
            )
