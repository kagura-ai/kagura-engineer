...
def setup(
    ...
    unsafe_auto_install: bool = typer.Option(False, "--unsafe-auto-install", help="allow sudo / privileged installers"),
...
def run_plan(steps: list[Step], *, no_input: bool, unsafe: bool) -> SetupReport:
    ...
```
...
|---|---|
| `git` on PATH | SKIPPED |
| 無い + `--unsafe-auto-install` | sudo pkg install |
| 無い + default | print `apt install -y git` (or brew/dnf/pacman/winget) → NEEDS_USER |
...
2. `platform.py` (OS / pkg manager detection)
3. `git.py` (最小の `shutil.which` パターン、後で他 step も真似る)
4. `claude.py` (credential cache 検出ロジックを Plan 1 と共有 helper に切り出し)
5. `gh.py` / `ollama.py` / `memory_cloud.py`
6. `__init__.py` orchestrator + `build_plan` + `run_plan`
7. CLI 配線 (`cli.py` の stub 削除 + 新 command)
8. Tests: 各 step 個別 (`monkeypatch` で `shutil.which` / `subprocess.run` / `Path.home` mock)、orchestrator は Plan 1 と同パターン
9. `--json` output test
10. Plan 3 (run) からの `setup --no-input` 接続点 placeholder

---

## 6. Open Questions 全 12 件 (他 plan 含めた統合一覧)

実装着手前に user spec call 必要:

| # | Plan | Question | 推奨 default |
|---|------|----------|-------------|
| Q1 | 2/3/5 | `workspace_id` を `Config` に追加? (Memory Cloud の `workspace_id → context_id → user_id` filter 階層) | **追加** |
| Q2 | 2/3/5 | Auth 解決順: env (`KAGURA_API_KEY`, `ANTHROPIC_API_KEY`, `GITHUB_TOKEN`) → keyring → Config フィールド? | **env 優先** |
| **Q2.1** | 2 | sudo auto-install default? | **print-only + opt-in** |
| **Q2.2** | 2 | `gh` auth no-TTY 時の挙動 | **fail loudly** |
| **Q2.3** | 2 | Ollama daemon 未起動 | **`ollama serve &` 試行 → fall back** |
| **Q2.4** | 2 | setup の re-runnability | **re-runnable** |
| **Q2.5** | 2 | `--fix <name>` 許可語彙 | **7 個** (Section 2) |
| **Q2.6** | 2 | `ANTHROPIC_API_KEY` probe | **不要** (サブスク基本) |
| Q3 | 3 | `claude -p` 起動戦略 | **subprocess 直叩き** (Agent SDK は将来 layer-on) |
| Q4 | 4 | Review gate default | **hard gate** + `--allow-exhausted` + `--interactive` |
| Q5 | 3 | `context_id` 寿命 | **per-project** (Config) + per-run sub-context 自動付与 |
| Q6 | 5 | Memory auto-store | **auto on** + `--no-remember` opt-out |
| Q7 | 5 | Offline mode | **`LocalMemoryClient` (SQLite) を protocol 2nd impl として用意** |
| Q8 | 5 | Memory privacy | **`trust_tier` フィールドで filter** (kagura-engineer 産出は `trusted`) |
| Q9 | 4 | `cfg.review.models` 空時の挙動 | **`run` で review 自体 skip** (Plan 3 fast path) |
| Q10 | 5 | Workspace bootstrap (`create_context`) を setup に含める? | **setup に含める** (Plan 2 step として) |
| Q11 | 3 | Run 単位 worktree 命名 | **`run-<short-id>`** (default) |
| Q12 | 3 | Memory recall budget | **2k tokens pinned + 4k on-demand `recall()`** |
...
- `Config.workspace_id` 追加 (Q1 spec call 待ち、Plan 2 着手時に同時追加はあり)

---

## 9. 完了条件 (Plan 2 Done)

- [ ] `setup` (no flag) で doctor の FAIL/WARN が 0 になる(repeatable)
