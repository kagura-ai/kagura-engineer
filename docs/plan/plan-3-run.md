...
1. **bounded composable + 単一 orchestrator**
   ...
   **`run` は他 agent(reviewer / planner)を束ねない。** workflow=gh-issue-driven が唯一の orchestration 層であり、`run` はそれを起動する actor 側ループ。reviewer(別プロダクト `kagura-code-review`)は PR 後に *別起動* で引き継ぐ — `run` は呼ばない。
...
| 項目 | 行き先 |
|---|---|
| standalone reviewer(`kagura-code-review`)連結・auto-review/auto-fix loop | Plan 4 |
...
- reviewer(`kagura-code-review`)連結・auto-review/fix loop(Plan 4)
